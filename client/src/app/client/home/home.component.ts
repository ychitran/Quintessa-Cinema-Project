...
export class HomeComponent implements OnInit {
  ...
  loadScreeningDate(event):void {
    this.homeService.getDate(event).subscribe(res => this.getDateDetail(res))
  }

  loadFilmDetail(id):void{
    this.homeService.getFilm(id).subscribe(res=> this.filmDetail(res));
  }
  filmDetail(res): void {
    this.film =  res[0];
  ...
  save() {
    ...
    this.homeService.save(value).subscribe(
      res =>
  ...
  loadStartTime(event):void {
    this.date = event;
    this.homeService.getStartTime(event).subscribe(res => this.getStartTimeDetail(res))
  }
  ...
  loadSeat(room_id, screening_id,start_time):void{
    ...
    this.homeService.getSeat(room_id,screening_id).subscribe(res=>this.getSeatDetail(res));
  }
    getSeatDetail(res) {
    this.seats=res;
  }
